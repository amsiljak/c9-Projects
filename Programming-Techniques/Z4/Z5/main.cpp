...
void Administracija::RegistrirajNoviLaptop(int ev_broj,std::string naziv,std::string karakt)
{
    ...
    laptopi.insert({ev_broj,new Laptop(ev_broj,naziv,karakt)});
}
void Administracija::RegistrirajNovogStudenta(int br,std::string god,std::string i_p,std::string adr,std::string br_tel)
{
    ...
    studenti.insert({br,new Student(br,god,i_p,adr,br_tel)});
}
...
Administracija::Administracija( Administracija &&adm)
{
    ...
    {
        studenti.insert({it->first,new Student(it->second->DajIndeks(),it->second->DajGodinuStudija(),it->second->DajImePrezime(),
        it->second->DajAdresu(),it->second->DajTelefon())});
        it->second=nullptr;
    }
    for(auto it=adm.laptopi.begin();it!=adm.laptopi.end();it++)
    {
        laptopi.insert({it->first,new Laptop(it->second->DajEvidencijskiBroj(),it->second->DajNaziv(),it->second->DajKarakteristike())});
    }
    }
    catch(...)
    {
...
Administracija &Administracija::operator =(const Administracija &adm)
{
    ...
    try{
    if(!studenti.count(it1->first)) studenti.insert({it1->first,new Student(it1->second->DajIndeks(),
    it1->second->DajGodinuStudija(),it1->second->DajImePrezime(),it1->second->DajAdresu(),it1->second->DajTelefon())}); 
    if(!laptopi.count(it2->first)) laptopi.insert({it2->first,new Laptop(it2->second->DajEvidencijskiBroj(),
...
{
    // std::string a,b,c,d;
    // a="1"; b="H m  "; c="Trg"; d="069/2-3"; 

    // Student s(35124,a,b,c,d);
    // s.Ispisi();
    // Administracija etf;
    // try
    // {
